...
object CompanionPersonalitySystem {
    ...
    fun dialogueFor(
        ...
        val careGuidance = when (starter.companion.species) {
            "Monstera" -> when (lowestNeedFor(careState)) {
            ...
            "Basil" -> when (lowestNeedFor(careState)) {
            ...
            "Tomato" -> when (lowestNeedFor(careState)) {
    ...
    private fun moodContextFor(
        ...
        lowestStatFor(careState) <= 40 -> CompanionMoodContext.NEEDY
    ...
    private fun lowestNeedFor(careState: PlantCareState): CareAction = when (minOf(careState.hydration, careState.sunlight, careState.nutrition)) {
        careState.hydration -> CareAction.WATER
        careState.sunlight -> CareAction.MOVE_TO_SUNLIGHT
        else -> CareAction.FERTILIZE
    }

    private fun lowestStatFor(careState: PlantCareState): Int = minOf(careState.hydration, careState.sunlight, careState.nutrition)
}
